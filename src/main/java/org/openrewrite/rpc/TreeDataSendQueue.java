...
public class TreeDataSendQueue {
    ...
    public <Parent, T> void value(@Nullable T after, Function<Parent, @Nullable T> beforeFn) {
        ...
        if (before == after) {
            ...
            put(new TreeDatum(TreeDatum.State.ADD, after));
        } else if (after == null) {
            put(new TreeDatum(TreeDatum.State.DELETE, null));
        } else {
            put(new TreeDatum(TreeDatum.State.CHANGE, after));
        }
    }
    ...
    public <T extends Tree> T tree(@Nullable T after, Consumer<@Nullable T> onChange) {
        if (before == after) {
            ...
            put(new TreeDatum(TreeDatum.State.CHANGE, null));
            //noinspection unchecked
            onChange.accept((T) before);
        }
    ...
    public <T> T value(@Nullable T after, @Nullable T before, Runnable onChange) {
        if (before == after) {
            ...
            put(new TreeDatum(TreeDatum.State.ADD, null));
        } else if (after == null) {
            put(new TreeDatum(TreeDatum.State.DELETE, null));
        } else {
            put(new TreeDatum(TreeDatum.State.CHANGE, null));
    ...
    public static <T> List<TreeDatum> listDifferences(@Nullable List<T> after,
        ...
        if (before == after) {
            ...
            for (T anAfter : after) {
                ...
                if (beforePos == null) {
                    data.add(new TreeDatum(TreeDatum.State.ADD, valueOnAdd.apply(anAfter)));
                } else {
